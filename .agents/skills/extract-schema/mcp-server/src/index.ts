...
**URL**: ${data.url}
**Total Blocks**: ${data.stats.totalBlocks}
**Formats Found**: JSON-LD (${data.stats.jsonLdBlocks}), Microdata (${data.stats.microdataBlocks}), RDFa (${data.stats.rdfaBlocks})
...
${Object.entries(
  ...
  .map(([type, blocks]) => `### ${type} (${blocks.length})\n${blocks.map(b => `- Format: ${b.format}`).join("\n")}`)
  .join("\n\n")}`;
